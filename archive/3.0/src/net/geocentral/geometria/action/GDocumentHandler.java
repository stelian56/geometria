...
public class GDocumentHandler implements GEventListener {
    ...
    public static GDocumentHandler getInstance() {
        if (instance == null) {
            ...
            try {
                ...
                instance.updateActionHandlerStates();
            }
    ...
    private void setPaths() {
        ...
        try {
            repositoryDir =
                new File(new File(".").getCanonicalPath(), "repository");
        }
        catch (Exception exception) {
            logger.error(exception);
            return;
        }
        figurePath = new File(repositoryDir,
                GDictionary.get("figures")).getPath();
        problemPath = new File(repositoryDir,
                GDictionary.get("problems")).getPath();
        solutionPath = new File(repositoryDir,
                GDictionary.get("solutions")).getPath();
    }
    ...
    public GFigure newFigure(GSolid solid, double zoomFactor) {
        ...
        String name = activeDocument.newFigureName();
        figure.setName(name);
    ...
    public GFileReader getFileReader(Frame ownerFrame, String filePath,
        ...
        return container.getFileReader(ownerFrame, filePath, filters,
                acceptAllFileFilter);
    }

    public GFileWriter getFileWriter(Frame ownerFrame, String filePath, 
            FileFilter[] filters, boolean acceptAllFileFilter) {
        return container.getFileWriter(ownerFrame, filePath, filters,
                acceptAllFileFilter);
    }
    ...
    private void updateTitleBar() {
        if (!(container instanceof GFrame))
            return;
        String barTitle = GDictionary.get(
