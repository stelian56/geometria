...
public class GRenamePointAction implements GUndoable, GFigureAction, GActionWithHelp {
    ...
    public void make(Element node) throws Exception {
        logger.info("");
        NodeList ns = node.getElementsByTagName("oldLabel");
        if (ns.getLength() == 0) {
            logger.error("No old label");
            throw new Exception();
        }
        oldLabel = ns.item(0).getTextContent();
        ns = node.getElementsByTagName("newLabel");
        if (ns.getLength() == 0) {
            logger.error("No new label");
            throw new Exception();
        }
        newLabel = ns.item(0).getTextContent();
