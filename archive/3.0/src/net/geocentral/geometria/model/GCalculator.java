    ...
    public GNotepadRecord evaluate(GDocument document) throws Exception {
        ...
        if (ts.length == 2) {
            ...
            if (!variableName.matches(GLabelFactory.VARIABLE_NAME_PATTERN)) {
                logger.info("Bad variable: " + input + ", " + variableName);
                throw new Exception(
            ...
            if (variables != null) {
                for (GNotepadVariable p : variables)
                    if (p.getName().equals(variableName)) {
                        logger.info("Duplicate variable: " + input + ", "
                                + variableName);
                        throw new Exception(
        ...
        try {
            ...
            select(expressionOffset
                    + tokenBounds.offset, expressionOffset
                    + tokenBounds.offset + tokenBounds.size);
            throw exception;
        }
        logger.info("Value: " + value);
        if (document != null && variableName != null) {
            List<GToken> tokens = evaluator.getPostfixTokens();
            Set<GNotepadVariable> params =
                new LinkedHashSet<GNotepadVariable>();
            for (GToken token : tokens) {
                if (token instanceof GVariable) {
                    GNotepadVariable param =
                        document.getVariable(((GVariable)token).getName());
                    params.add(param);
                }
            }
            expression = expression.replaceAll(" +", "");
            GCalculation calculation = new GCalculation(expression);
            GNotepadVariable variable = new GNotepadVariable(variableName,
                    value);
