...
public abstract class GDocument extends GXmlEntity {
    ...
    public String newFigureName() {
        logger.info("");
        String prefix = GDictionary.get("Figure");
        ...
        int suffix = suffixes.isEmpty() ?
                1 : suffixes.get(suffixes.size() - 1) + 1;
    ...
    public String[] getFigureNames() {
        String[] figureNames = new String[figures.size()];
        int index = 0;
        for (String figureName : figures.keySet())
            figureNames[index++] = figureName;
        return figureNames;
    }
