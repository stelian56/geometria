...
public class GParallelToPlaneCondition implements GPlaneCondition {
    ...
    public void validate(String valueString, GDocument document)
        ...
        Point3d[] cs = GPointSetUtils.fromString(valueString,
                figure);
        if (cs.length != 3) {
            logger.info(cs.length);
            throw new Exception(GDictionary.get("Enter3RefPoints"));
        }
        coords = cs;
    }
    ...
    public boolean verify(Point3d[] cs, GDocument document) {
        ...
        Vector3d n1 = GMath.cross(cs[0], cs[1], cs[2]);
        n1.normalize();
        Vector3d n2 = GMath.cross(coords[0], coords[1], coords[2]);
        n2.normalize();
        return GMath.areCollinear(n1, n2, GMath.EPSILON);
    }

    public void serialize(StringBuffer buf) {
        logger.info("");
        buf.append("\n<condition>");
        buf.append("\n<type>parallelToPlane</type>");
    ...
    public String getDescription() {
        return GDictionary.get("ParallelToPlane");
    }
}
